...
const Advertisement = () => {
  ...
  useEffect(() => {
    const adData = async () => {
      ...
      if (resData?.data) {
        ...
        console.log(resData?.data, advertisementList, "resData------");
      }
    };
    adData();
  }, [save]);

  // console.log(advertisementList, "sssssssssssss");
  ...
  console.log(advertisementList?.data, "advertisementList?.data");
  useEffect(() => {
          ...
          if (section.title === "Details") {
            const updatedFields = section.fields.map((field, index) => {
              if (index === 0) {
                return {
                  ...field,
                  options: advertisers_list?.data?.map((ele) => ({
                    label: ele.name + " ( " + ele?.company_name + " ) ",
                    value: ele.id,
                  })),
                };
              }
