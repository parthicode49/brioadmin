...
const AdInEpisode = ({ id }) => {
  ...
  const {
    ...
    advertisement_in_episode_list_admin,
  ...
  useEffect(() => {
    if (id) {
      const apiData = async () => {
        const resData = await advertisement_in_episode_list_admin({
          episode_id: id,
        });
  ...
  const [tableData, setTableData] = useState({
    ...
    deleteRecord: Action.advertisement_in_episode_delete,
